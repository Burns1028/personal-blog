...
日期：2026-08-09
状态：已实施，production revision 8

## 0. 用户最终修订

本节记录生成与发布期间的最终偏好，并覆盖下文中与其冲突的早期约束：

- 用户提供本人照片，并明确授权三张人物图以本人脸部为身份参考。
- 人物默认采用侧脸或背侧脸，避免正脸、直视镜头和写真式摆拍；识别度主要来自发型、圆框眼镜、鼻梁与下颌轮廓。
- 最终资源使用 `17-legacy-service-night-office-image2-v2.png`、`18-conversion-rehearsal-image2-v2.png`、`19-weekend-agent-prototyping-image2-v2.png`。
- 公开图注只保留图片与正文的关系，不出现“AI 生成”“生成的情境插图”或“概念示意”等制作方式。
- 三张场景仍不能被正文描述为当时实拍或现场证据。
...
6. 本地构建并检查桌面与移动端文章页面。
7. 发布为文章的新修订版本。最终生产版本为 revision 8；revision 7 是用户补充本人身份参考与图注偏好之前的中间版本。
