...
首次内容迁移不复制本地 SQLite 文件到服务器，也不把 Markdown、灵感记录或文章媒体提交进代码仓库。Git 只负责代码；待迁移数据单独保存在本机忽略目录，并由一份明确的选择清单控制范围。

首批生产数据只包含：

- 文章 `taste-is-all-you-need`；
- 灵感 `2026-08-02-camus-meursault-starry-night`；
- 灵感 `2026-08-02-ai-era-no-moat`；
- 灵感 `2026-08-02-speak-loudly-small-ego`。

本地 SQLite 中其余四条历史或测试文章不上传、不进入 Git，也不在迁移过程中删除。部署 API 后，只把选择清单中的内容分别通过两个远程 Skill 写入生产数据库。Projects 与 Activities 不混入这两个接口，继续由独立的项目进度发布链管理。生产 SQLite 从第一天就是独立事实源。
